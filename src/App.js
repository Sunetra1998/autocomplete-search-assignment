...
function App() {
  ...
  const [selectedBooksHistory, setSelectedBooksHistory] = useState([]);
  ...
  const handleSearch = (query) => {
    const lowerSearchTerm = query.toLowerCase();
    const results = data.titles.filter(title => title.toLowerCase().includes(lowerSearchTerm));
    setSearchResults(results);
  };

  const handleSelectBook = (book) => {
    const index = data.titles.indexOf(book);
    setSelectedBook(book);
    setSelectedAuthor(data?.authors[index]?.author);
    setSelectedSummary(data?.summaries[index]?.summary);
    setSelectedBooksHistory(prevHistory => {
      const newHistory = [...prevHistory.slice(-7), data.titles[index]]; 
      return newHistory;
    });
  };
    ...
    <div className="App">
      ...
      <div className='SearchResults'>
        <ul>
          {searchResults.map((result) => {
            return (
              <li key={result.id} onClick={() => handleSelectBook(result)}>
                {result}
              </li>
            )
          }
      ...
      <div className='SearchResultsContainer'>
        {searchResults.length > 0 && (
          <>
            Search Results...
            <SearchResults searchResults={selectedBooksHistory} author={selectedAuthor}
              summary={selectedSummary} />
          </>
        )}
      </div>
